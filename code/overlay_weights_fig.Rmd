...
ggplot() +
  ...
  theme_void() +
  labs(alpha = "Percentage of Cell Covered 
by Secondary Weights Raster:") +
...
ggplot() +
  ...
  geom_tile(data = overlay, mapping = aes(x = x, y = y, alpha = 100*w_area), fill = w_area_fill, size = 2, color = grid_color) +
  scale_alpha_continuous(range = c(0,.5)) +
  theme_void() +
  labs(alpha = "Percentage of Polygon Captured by Cell:") +
  theme(legend.position = "bottom",
        legend.direction = "horizontal")
```
...
ggplot() +
  ...
  theme_void() +
  labs(alpha = "Area Normalized Secondary Weights:") +
  theme(legend.position = "bottom")
```

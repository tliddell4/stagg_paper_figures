...
temp_poly <- rbind(fread(here("data/temp_polynomial_2000_2004_US_counties_ERA5_pop_weights.csv")),
...
formula_poly <- as.formula(paste0("log(mortality) ~ ", poly_string, "+ order_1_prcp + order_2_prcp | as.factor(poly_id):as.factor(month) + as.factor(statefp):as.factor(year) | 0 | statefp"))

poly_regression <- felm(formula_poly, all_data)
...
plot_data_bin <- tidy(bin_regression, conf.int = TRUE)

plot_data_bin <- plot_data_bin %>% 
  filter(term %in% bins) %>% 
...
ggplot(data = plot_data_bin) +
  ...
  scale_x_continuous(labels = c("<n10", "n10-n5", "n5-0", "0-5", "5-10",
                     ...
                     breaks = c(-12.5,-7.5, -2.5, 2.5, 7.5, 12.5, 17.5, 22.5, 27.5,32.5),
                     expand = c(0,0)) +
  ...
  geom_function(fun = poly_plot_function, color = "blue")


plot_data_poly <- tidy(poly_regression, conf.int = TRUE)

poly_plot_function <- function(x){plot_data_poly$estimate[4]*(x)^4 + 
                                  plot_data_poly$estimate[3]*x^3 +
                                  plot_data_poly$estimate[2]*x^2 +
                                  plot_data_poly$estimate[1]*x -
                                  plot_data_poly$estimate[4]*17.5^4 - 
                                  plot_data_poly$estimate[3]*17.5^3 -
                                  plot_data_poly$estimate[2]*17.5^2 -
                                  plot_data_poly$estimate[1]*17.5}

easy_function <- function(x){x^2}

ggplot() + 
  geom_function(fun = poly_plot_function) +

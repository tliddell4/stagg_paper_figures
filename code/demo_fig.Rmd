...
bin_formula <- as.formula(paste0("log(mortality) ~ ", bin_string, " + order_1_prcp + order_2_prcp | as.factor(poly_id):as.factor(month) + as.factor(statefp):as.factor(year) | 0 | statefp"))
...
poly_formula <- as.formula(paste0("log(mortality) ~ ", poly_string, "+ order_1_prcp + order_2_prcp | as.factor(poly_id):as.factor(month) + as.factor(statefp):as.factor(year) | 0 | statefp"))

poly_regression <- felm(poly_formula, all_data)

spline_formula <- as.formula(paste0("log(mortality) ~ value + term_1 + order_1_prcp + order_2_prcp | as.factor(poly_id):as.factor(month) + as.factor(statefp):as.factor(year) | 0 | statefp"))
...
ggplot() +
  ...
  geom_line(data = bin_plot_data, mapping = aes(x = graph_index, y = estimate), size = 1) +
  geom_point(data = bin_plot_data, mapping = aes(x = graph_index, y = estimate), color = "gold") +
  theme_minimal() +
  ...
  geom_function(fun = spline_plot_function, color = "red")

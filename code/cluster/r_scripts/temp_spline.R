...
# Stagg Variables
knot_locations <- c(-2.5, 17.5, 27.5)
year_start <- 2000
year_end <- 2019
...
# Load in Data ---------------------------------------------------------------------------
# Read in Polygons
load(here("data", "counties.Rda")) # Named counties
counties <- st_as_sf(counties) %>% 
  #filter(GEOID %in% c("06083", "06111", "06079")) %>% 
  st_as_sf()
...
run_stagg <- function(year) {  
  ...
  ## run stagg for temp
  output <- staggregate_spline(clim_raster_tmp,
                                     area_weights,
                                     daily_agg = 'average',
                                     time_agg = 'month',
                                     knot_locs = knot_locations)
  return(output)
...
## run 
stagg_multiyear_temp <- parLapply(cl, years, run_stagg)
...
## save outputs
save_name <- paste0(paste("temp", "spline", year_start, year_end, "US", "counties", "ERA5",
                          "pop_weights", sep="_"), ".csv")


output_save_path <- "/home/tliddell/stagg_paper_figures/data/stagg_output/temperature/spline"
## save message

...
cropland_fig <- ggplot() +
  ...
  geom_tile(data = cropland_table_crop, mapping = aes(x = x, y = y), 
            alpha = 0, lwd = 0.6, color = grid_color)  +
  geom_sf(data = county, alpha = 0, lwd = 0.5) +
...
resamp_sec_weights_fig <- ggplot() +
  ...
  geom_sf(data = county, alpha = 0, lwd = poly_width, color = county_color) +
...
admin_region_fig <- 
  ...
  geom_tile(data = cropland_table_crop, mapping = aes(x = x, y = y), alpha = 0, size = 0.5, color = grid_color) +
  geom_sf(data = county, fill = w_area_fill, color = county_color, alpha = 0.6, lwd = poly_width) +
  theme_void() +
...
weight_area_fig <-
  ...
  scale_fill_gradient(low = "white",
                      ...
                      guide = guide_colorbar(frame.colour = "black",
                                             ticks.colour = "black",
                                             ticks = TRUE)) +
  geom_sf(data = st_shift_longitude(county), alpha = 0, lwd = poly_width, color = county_color) +
  theme_void() +
...
combined_weight_fig <- 
  ggplot() +
  geom_tile(data = overlay, mapping = aes(x = x, y = y, fill = combined_weight), lwd = 0.3, color = grid_color) +
  scale_fill_gradient(low = "white",
                      high = combined_fill,
